...
| 操作 | 前提条件 | 変更内容 |
|---|---|---|
| 田んぼを作る | なし | `Paddy`、いちばん上の田、空の主題、空欄のマス3つを作る |
| 書く | なし | マスが空欄なら `Content` を作る。あれば `text` を更新する |
| 型を変える | 田が未完 | `kindRaw` を更新する |
...
| 発展 | 田が完成している | 新しい型の田を作り、`evolvedFrom` で元の田とつなぐ（置き場所は未決） |
...
- **発展した田の置き場所**：新しい田んぼのいちばん上に置くか、元の田と同じ田んぼの中に置くか。
- **元に戻す操作**：田を消したときなどに、元に戻す（Undo）やゴミ箱を用意するか。SwiftData の `UndoManager` 連携で実現できる。
- **Three Good Things の主題**：作成時に日付を自動で入れるか。
